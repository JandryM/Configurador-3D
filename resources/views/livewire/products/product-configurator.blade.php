<div class="w-full px-2 sm:px-4 lg:px-8 py-8 bg-gray-50 text-black pt-24">
        <!-- Título -->
        <div class="mb-8">
            <div class="flex items-start gap-6">
                <div class="flex-1">
                    ...
                    <p class="text-black mt-2">Personaliza tu producto y ve los cambios en tiempo real</p>
                    <div class="mt-3 flex items-center gap-4 text-sm text-black">
                        <span class="inline-flex items-center px-2 py-1 rounded-full bg-blue-100 text-black">
                            {{ $product->category?->name ?? 'Sin categoría' }}
                        </span>
                        <span class="inline-flex items-center px-2 py-1 rounded-full bg-green-100 text-black">
        ...
        @if (session()->has('message'))
            <div class="mb-6 rounded-md bg-green-50 p-4">
                <p class="text-sm font-medium text-black">{{ session('message') }}</p>
            </div>
        @endif
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h2 class="text-lg font-semibold text-black mb-4">{{ $product->name }}</h2>
                    <div class="flex items-center gap-4">
                        <div class="flex-1">
                            <p class="text-sm text-black mt-1">{{ Str::limit($product->description, 120) }}</p>
                            <div class="mt-2 flex items-center gap-2">
                                <span class="text-lg font-bold text-black">${{ number_format($calculatedPrice, 2) }}</span>
                                <span class="text-sm text-black">precio calculado por materiales</span>
                            </div>
                            <div class="mt-2 flex items-center gap-2">
                                <span class="inline-flex items-center px-2 py-1 rounded-full bg-purple-100 text-black text-xs">
                                    🎯 Modelo 3D Generativo
                                </span>
                                <span class="inline-flex items-center px-2 py-1 rounded-full bg-blue-100 text-black text-xs">
                                    🚀 Tiempo Real
                                </span>
                            </div>
                        </div>
                ...
                <!-- Configurador 3D -->
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h2 class="text-xl font-semibold text-black mb-4">Vista 3D Interactiva</h2>
                    ...
                    <div class="mt-4 p-4 bg-gray-50 rounded-lg">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm text-black">
                            <div>
                                <span class="font-medium text-black">Ancho:</span>
                                <span class="text-black">{{ number_format($parameters['width'], 2) }}m</span>
                            </div>
                            <div>
                                <span class="font-medium text-black">Alto:</span>
                                <span class="text-black">{{ number_format($parameters['height'], 2) }}m</span>
                            </div>
                            <div>
                                <span class="font-medium text-black">Área:</span>
                                <span class="text-black">{{ number_format($parameters['width'] * $parameters['height'], 2) }}m²</span>
                            </div>
                            <div>
                                <span class="font-medium text-black">Precio:</span>
                                <span class="text-black font-bold">${{ number_format($calculatedPrice, 2) }}</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="space-y-6">
                <!-- Dimensiones -->
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h3 class="text-lg font-semibold text-black mb-4">Dimensiones</h3>
                ...
                <!-- Colores -->
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h3 class="text-lg font-semibold text-black mb-4">Colores Disponibles</h3>
                    
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-black mb-2">Color del Aluminio</label>
                            ...
                            @if($selectedColor)
                            <div class="mt-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border border-blue-200 text-black">
                        ...
                        <div class="mt-8">
                            <label class="block text-sm font-medium text-black mb-2">Color del Vidrio</label>
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                @foreach($availableColors as $colorName => $color)
                                    @if(str_contains(strtolower($colorName), 'glass') || str_contains(strtolower($colorName), 'vidrio'))
                                    <div class="relative">
                ...
                <!-- Precio y acciones -->
                <div class="bg-white rounded-lg shadow-lg p-6">
                    <h3 class="text-lg font-semibold text-black mb-4">Precio Estimado</h3>
                    ...
                    <div class="space-y-3">
                        ...
                        @else
                            <div class="w-full bg-yellow-100 text-black font-medium py-2 px-4 rounded-lg text-center">
...
@if($showProformaModal ?? false)
    @if(Auth::check())
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full p-6 relative text-black">
                ...
                <h2 class="text-xl font-bold mb-4 text-center text-black">Proforma de Producto</h2>
                @if (session()->has('message'))
                    <div x-data="{ show: true }" x-show="show" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-90" x-transition:enter-end="opacity-100 scale-100" x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100 scale-100" x-transition:leave-end="opacity-0 scale-90" class="fixed inset-0 z-60 flex items-center justify-center bg-black bg-opacity-40">
                        <div class="bg-white rounded-lg shadow-2xl max-w-sm w-full p-6 relative border-2 border-green-500 text-black">
                            ...
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-black text-center">{{ session('message') }}</p>
                            </div>
                        </div>
                    </div>
                @endif
    ...
    @else
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40">
            <div class="bg-white rounded-lg shadow-xl max-w-md w-full p-8 relative flex flex-col items-center">
                ...
                <h3 class="text-xl font-bold text-black mb-2">Acceso restringido</h3>
                <p class="text-black text-center">Debes iniciar sesión para visualizar, guardar o descargar la proforma.</p>
            </div>
        </div>
    @endif
@endif
